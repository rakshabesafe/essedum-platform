...
export class ConnectionViewComponent implements OnInit{
  ...
  ngOnInit() {
    this.router.url.includes('initiative')? (this.initiativeView = false): (this.initiativeView = true);
        this.service.getPermission("cip").subscribe((cipAuthority)=>{
          ...
          cipAuthority.includes('connection-unlink')
          )
    ...
    if(this.router.url.includes('preview')){
      ...
      this.view=true;
      this.component=[];
      this.service.getRelatedComponent(this.data.id,'CONNECTION').subscribe({next:res => {
    ...
    this.route.params.subscribe( params => this.connectionName = params.name );
  ...
  editConnection() {
     if(this.edit){
      ...
      this.Services.getCoreDatasource(this.connectionName,org).subscribe(res=>{
         this.data =res;
  ...
  getDatasourcePorts(id:any){
    this.Services.getDatasourcePort(id).subscribe(res=>{
      ...
      console.log("This is datasource ports details : ",this.datasourceportsjson);
    });
    ...
    try{
      ...
      this.Services.getPluginsLength().
        subscribe(
          ...
          () => {
            this.Services.getDatasourceJson(0, size) .subscribe(res => {
