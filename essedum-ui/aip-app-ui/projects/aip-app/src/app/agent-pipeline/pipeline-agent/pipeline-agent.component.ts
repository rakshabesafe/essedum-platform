...
@Component({
  selector: 'app-pipeline-agent',
  templateUrl: './pipeline-agent.component.html',
  styleUrls: ['./pipeline-agent.component.scss'],
})
  ...
  redirection(card: any): void {
    this.service.getStreamingServicesByName(card.name).subscribe((res) => {
      ...
      if (this.streamItem.type === 'AIAgent') {
        this.router.navigate(['./pipeline-detail' + '/' + card.name], navigationExtras);
      }
    });
  }
