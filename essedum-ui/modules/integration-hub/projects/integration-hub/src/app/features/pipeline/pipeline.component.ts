...
export class PipelineComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Cross-MFE: when data-ops emits DATASET_CREATED or agent emits AGENT_DEPLOYED,
    // refresh this view so the new entity is immediately reflected in pickers/lists.
    this.busSubs.add(this.eventBus.on('DATASET_CREATED').subscribe(() => this.refresh()));
    this.busSubs.add(this.eventBus.on('AGENT_DEPLOYED').subscribe(() => this.refresh()));
