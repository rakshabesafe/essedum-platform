...
import { SwaggerAPISpec, EventBusService } from '@essedum/shared-lib';
  ...
  createDataset() {
    try {
      ...
      this.busy = this.datasetsService.createDataset(dataset).subscribe((res) => {
        ...
        // Cross-MFE event: integration/agent MFEs can subscribe to refresh their dataset pickers.
        this.eventBus.emit({ type: 'DATASET_CREATED', payload: { id: res?.id ?? returnedName, name: returnedName } });
