...
socketio = SocketIO(
    ...
    async_mode="eventlet",
    ping_interval=25,
    ping_timeout=300,      # 5 minutes for long builds
    logger=True,
    engineio_logger=True,
    async_handlers=True,   # Allow long-running handlers
    ...
    try:
        ...
        if not dockerfile_exists:
            ...
            if os.path.exists(fallback_dockerfile):
                # Auto-detect the main Python entry point
                entry_point = _detect_python_entrypoint(build_context_path)
                log_to_client(f"Detected entry point: {entry_point}", step="PREP")

                with open(fallback_dockerfile, "r", encoding="utf-8") as _f:
                    fallback_content = _f.read()

                # Replace the hardcoded CMD with the detected entry point
                import re as _re
                fallback_content = _re.sub(
                    r'^CMD\s+\[.*\]\s*$',
                ...
                dest_dockerfile = os.path.join(build_context_path, "Dockerfile")
                with open(dest_dockerfile, "w", encoding="utf-8") as _f:
                    _f.write(fallback_content)
                log_to_client(f"Dockerfile missing; fallback applied with entry point: {entry_point}", step="PREP")
            else:
        ...
        # Detect the app port from source code
        detected_port = _detect_app_port(build_context_path, entry_point)
        log_to_client(f"Detected app port: {detected_port}", step="PREP")
        ...
        try:
            ...
            k8s_core.patch_namespaced_service(
                name=deploy_name, namespace=target_namespace,
                body={"spec": {"ports": [{"name":"http","port":80,"targetPort":detected_port}],
            ...
            if e.status == 404:
                ...
                svc = client.V1Service(
                    ...
                    spec=client.V1ServiceSpec(
                        selector={"app": deploy_name},
                        ports=[client.V1ServicePort(port=80, target_port=detected_port)],
                        type="ClusterIP" #
                    )
                )
...
def _detect_python_entrypoint(directory):
    """
    Detect the main Python entry point file in a directory.
    Priority:
      1. Common well-known names: app.py, main.py, run.py, server.py
      2. Any .py file at the top level containing 'if __name__ == "__main__"'
      3. Any single top-level .py file
      4. Falls back to 'app.py' if nothing found
    """
    preferred = ["app.py", "main.py", "run.py", "server.py"]
    py_files = [f for f in os.listdir(directory) if f.endswith(".py") and os.path.isfile(os.path.join(directory, f))]
    ...
    for py_file in py_files:
        try:
            with open(os.path.join(directory, py_file), "r", encoding="utf-8", errors="ignore") as f:
...
def _detect_app_port(directory, entry_point):
    """
    Try to detect the port the application listens on by scanning source files.
    Looks for common patterns like port=NNNN, .run(port=NNNN), HOST:PORT, etc.
    Returns the detected port as an int, or 5000 as the default.
    """
    import re as _re

    # Scan entry point first, then all .py files
    files_to_scan = []
    if entry_point:
        ep_path = os.path.join(directory, entry_point)
        if os.path.isfile(ep_path):
            files_to_scan.append(ep_path)
    for f in os.listdir(directory):
        full = os.path.join(directory, f)
        if f.endswith(".py") and os.path.isfile(full) and full not in files_to_scan:
            files_to_scan.append(full)

    port_patterns = [
        # uvicorn.run(..., port=8008)
        _re.compile(r'(?:uvicorn\.run|app\.run|run_server|serve)\s*\([^)]*port\s*=\s*(\d{2,5})', _re.IGNORECASE),
        # argparse default: default=8008 after "port"
        _re.compile(r'["\']--?port["\'][^)]*default\s*=\s*(\d{2,5})'),
        # PORT = 8008  or  port = 8008
        _re.compile(r'\bport\s*=\s*(\d{2,5})\b', _re.IGNORECASE),
        # 0.0.0.0:8008 or host:8008 string literal
        _re.compile(r'["\'](?:0\.0\.0\.0|localhost|127\.0\.0\.1):(\d{2,5})["\']'),
    ]
