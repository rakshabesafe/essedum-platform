# Python Job Executor for Azure

This directory contains the source code for the Python Job Executor, which is a component of the Essedum platform responsible for executing jobs on Azure.

## Overview

The Python Job Executor for Azure is a separate service that listens for job requests from the backend and executes them in a controlled environment on Azure. This service is essential for running data processing, machine learning, and other Python-based tasks within the Essedum ecosystem on the Azure cloud.
...
- **Azure CLI**: The Azure Command-Line Interface for interacting with Azure resources.
...
2. **Create a Virtual Environment**:
   - Navigate to the `py-job-azure-executer` directory.
...
2. **Install the Service**:
   - Open a command prompt with administrator privileges and run:
     ```bash
     nssm install "Python Job Executor for Azure"
     ```
   - In the NSSM GUI, configure the following:
     - **Path**: The full path to your `python.exe` (e.g., `C:\path\to\your\venv\Scripts\python.exe`).
     - **Startup directory**: The path to the `py-job-azure-executer` directory.
     - **Arguments**: `app.py`.
     - **I/O Tab**: Redirect stdout and stderr to a log file for debugging.

3. **Start the Service**:
   - You can now start, stop, and manage the "Python Job Executor for Azure" service from the Windows Services panel (`services.msc`).
