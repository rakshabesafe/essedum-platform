...
public interface ICIPMLFederatedModelsRepositoryMSSQL  extends ICIPMLFederatedModelsRepository{
	...
	@Query(value="SELECT t1.id, " + "t1.fed_id, " + "t1.adapter_id, " + "t1.app_name, " + "t1.fed_name, "
			...
			+ " AND t1.app_org= :project AND (concat('',:adaptertype,'')='notRequired' OR CHARINDEX(',' + t1.model_type + ',', ',' + :adaptertype + ',') > 0) AND "
			+ "(concat('',:adapterinstance,'')='notRequired' OR CHARINDEX(',' + t1.adapter_id + ',', ',' + :adapterinstance + ',') > 0 ) AND"
			+ " (  t2.tag_id IN ( :tags)) AND"
			+ "  (:query1 IS NULL OR t1.app_name like CONCAT('%', :query1, '%')  OR t1.raw_payload like  CONCAT('%', :query1, '%') "
			+ "  OR t1.model_type like  CONCAT('%', :query1, '%') OR t1.app_description like  CONCAT('%', :query1, '%') OR t1.app_status like  CONCAT('%', :query1, '%') )"
			+ " ORDER BY COALESCE(TRY_CONVERT(datetime, t1.app_modified_date), t1.fed_modified_date ) DESC",nativeQuery = true)
	List<ICIPMLFederatedModel> getAllModelsByAppOrgandAdapterIdWithTag(@Param("project") String project,
			@Param("tags") List<Integer> tags, Pageable paginate, @Param("query1") String query,
			@Param("adapterinstance") String adapterInstance, @Param("adaptertype") String type);
	
	@Query(value="SELECT * from mlfederatedmodels t1 WHERE t1.is_deleted = 0 AND"
			+ " t1.app_org= :project AND (concat('',:adaptertype,'')='notRequired' OR CHARINDEX(',' + t1.model_type + ',', ',' + :adaptertype + ',') > 0) AND "
			+ "(concat('',:adapterinstance,'')='notRequired' OR CHARINDEX(',' + t1.adapter_id + ',', ',' + :adapterinstance + ',') > 0 ) AND"
			+ "  (:query1 IS NULL OR t1.app_name like  CONCAT('%', :query1, '%') OR t1.raw_payload like  CONCAT('%', :query1, '%') "
			+ "  OR t1.model_type like  CONCAT('%', :query1, '%') OR t1.app_description like  CONCAT('%', :query1, '%') OR t1.app_status like  CONCAT('%', :query1, '%'))"
			+ " ORDER BY COALESCE(TRY_CONVERT(datetime, t1.app_modified_date), t1.fed_modified_date ) DESC",nativeQuery = true)
	List<ICIPMLFederatedModel> getAllModelsByAppOrgandAdapterIdWithoutTag(@Param("project") String project,
			Pageable paginate, @Param("query1") String query, @Param("adapterinstance") String adapterInstance,
	...
	Long getAllModelsCountByAppOrgandAdapterIdWithTag(@Param("project") String project,
			@Param("tags") List<Integer> tagList, @Param("query1") String query,
			@Param("adapterinstance") String instance, @Param("adaptertype") String type);

   	@Query(value="SELECT count(*) from mlfederatedmodels t1 WHERE t1.is_deleted = 0 AND"
	            + " t1.app_org= :project AND (concat('',:adaptertype,'')='notRequired' OR CHARINDEX(',' + t1.model_type + ',', ',' + :adaptertype + ',') > 0) AND "
			+ "(concat('',:adapterinstance,'')='notRequired' OR CHARINDEX(',' + t1.adapter_id + ',', ',' + :adapterinstance + ',') > 0 ) AND"
			+ "  (:query1 IS NULL OR t1.app_name like  CONCAT('%', :query1, '%') OR t1.raw_payload like  CONCAT('%', :query1, '%') "
			+ "  OR t1.model_type like  CONCAT('%', :query1, '%') OR t1.app_description like  CONCAT('%', :query1, '%') OR t1.app_status like  CONCAT('%', :query1, '%'))",nativeQuery = true)
	...
	@Query(value="SELECT * FROM mlfederatedmodels WHERE fed_id IN(SELECT DISTINCT fed_id FROM mlfederatedmodels WHERE app_org = :org and app_name like CONCAT('%',:search,'%'))",nativeQuery = true)

	List<ICIPMLFederatedModel> getAllDistinctModelsByAppOrg(@Param("org")String organization,
