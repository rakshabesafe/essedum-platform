...
public interface ICIPMLFederatedModelsRepositoryMYSQL extends ICIPMLFederatedModelsRepository {
	...
	List<ICIPMLFederatedModel> getAllModelsByAppOrgandAdapterId(@Param("org") String org,
	...
	@Query(value = "SELECT t1.id, " + "t1.fed_id, " + "t1.adapter_id, " + "t1.app_name, " + "t1.fed_name, "
			...
			+ " ORDER BY COALESCE(t1.app_modified_date, t1.fed_modified_date ) DESC", nativeQuery = true)
	...
	@Query(value = "SELECT * from mlfederatedmodels t1 WHERE is_deleted = FALSE AND"
	...
	@Query(value = "SELECT count(*) from mlfederatedmodels t1 WHERE is_deleted = FALSE AND"
			+ " t1.app_org= :project AND (concat('',:adaptertype,'')='notRequired' OR FIND_IN_SET(t1.model_type, :adaptertype)) AND "
			+ "(concat('',:adapterinstance,'')='notRequired' OR   FIND_IN_SET (t1.adapter_id,:adapterinstance)) AND"
			+ "  (:query1 IS NULL OR t1.app_name like  CONCAT('%', :query1, '%') OR t1.raw_payload like  CONCAT('%', :query1, '%') "
			+ "  OR t1.model_type like  CONCAT('%', :query1, '%') OR t1.app_description like  CONCAT('%', :query1, '%') OR t1.app_status like  CONCAT('%', :query1, '%'))", nativeQuery = true)
	...
	List<ICIPMLFederatedModel> getModelByFedNameAndOrg(@Param("fedName") String fedName, @Param("org") String org);
