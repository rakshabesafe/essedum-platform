...
@Profile("mssql")
@Repository
public interface ICIPMLFederatedEndpointRepositoryMSSQL extends ICIPMLFederatedEndpointRepository{
	...
	@Query(value="SELECT t1.id, " + "t1.fed_id, " + "t1.adapter_id, " + "t1.app_name, " + "t1.fed_name, "
			...
			+ " AND t1.app_org= :project AND (concat('',:adaptertype,'')='notRequired' OR CHARINDEX(',' + t1.endpoint_type + ',', ',' + :adaptertype + ',') > 0) AND "
			...
			+ "  OR t1.endpoint_type like  CONCAT('%', :query1, '%') OR t1.app_status like  CONCAT('%', :query1, '%') ) ORDER BY COALESCE(TRY_CONVERT(datetime, t1.app_modified_date), t1.fed_modified_date ) DESC",nativeQuery = true)
...
//			@Param("query1") String query, @Param("adapterinstance") String adapterInstance,@Param("adaptertype")String type);
	...
	@Query(value="SELECT * from mlfederatedendpoints t1 WHERE "
			+ " t1.app_org= :project AND (concat('',:adaptertype,'')='notRequired' OR CHARINDEX(',' + t1.endpoint_type + ',', ',' + :adaptertype + ',') > 0) AND "
			+ "(concat('',:adapterinstance,'')='notRequired' OR CHARINDEX(',' + t1.adapter_id + ',', ',' + :adapterinstance + ',') > 0) AND"
			+ "  (:query1 IS NULL OR t1.app_name like  CONCAT('%', :query1, '%') OR t1.raw_payload like  CONCAT('%', :query1, '%') "
	...
	@Query(value="SELECT * from mlfederatedendpoints t1 WHERE t1.is_deleted = 0 AND "
			+ " t1.app_org= :project AND (concat('',:adaptertype,'')='notRequired' OR CHARINDEX(',' + t1.endpoint_type + ',', ',' + :adaptertype + ',') > 0) AND "
			+ "(concat('',:adapterinstance,'')='notRequired' OR CHARINDEX(',' + t1.adapter_id + ',', ',' + :adapterinstance + ',') > 0) AND"
			+ "  (:query1 IS NULL OR t1.app_name like  CONCAT('%', :query1, '%') OR t1.raw_payload like  CONCAT('%', :query1, '%') "
			+ "  OR t1.endpoint_type like  CONCAT('%', :query1, '%') OR t1.app_status like  CONCAT('%', :query1, '%') )  ORDER BY COALESCE(TRY_CONVERT(datetime, t1.app_modified_date), t1.fed_modified_date ) DESC",nativeQuery = true)
	...
	@Query(value="SELECT count(*) from mlfederatedendpoints t1 WHERE t1.is_deleted = 0 AND "
			+ " t1.app_org= :project AND (concat('',:adaptertype,'')='notRequired' OR CHARINDEX(',' + t1.endpoint_type + ',', ',' + :adaptertype + ',') > 0) AND "
			+ "(concat('',:adapterinstance,'')='notRequired' OR CHARINDEX(',' + t1.adapter_id + ',', ',' + :adapterinstance + ',') > 0) AND"
			+ "  (:query1 IS NULL OR t1.app_name like  CONCAT('%', :query1, '%') OR t1.raw_payload like  CONCAT('%', :query1, '%') "
			+ "  OR t1.endpoint_type like  CONCAT('%', :query1, '%') OR t1.app_status like  CONCAT('%', :query1, '%') )",nativeQuery = true)
