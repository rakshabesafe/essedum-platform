...
import com.lfn.icip.dataset.repository.ICIPDatasetIdsmlRepository;

@Profile("mssql")
@Repository
public interface ICIPDatasetIdsmlRepositoryMSSQL extends ICIPDatasetIdsmlRepository{

	@Query(value="SELECT * FROM mldatasetidsml WHERE dataset = :datasetid AND org = :org AND is_chart = 1",nativeQuery = true)
