...
from langflow.services.mcp import initialize_mysql_mcp_service
    ...
    async def lifespan(_app: FastAPI):
        ...
        try:
            ...
            async def delayed_init_mcp_servers():
                ...
                # Initialize MySQL MCP service
                try:
                    await logger.adebug("Initializing MySQL MCP service")
                    await initialize_mysql_mcp_service()
                    await logger.ainfo("MySQL MCP service initialized successfully")
                except Exception as e:  # noqa: BLE001
                    await logger.awarning(f"Failed to initialize MySQL MCP service: {e}")
