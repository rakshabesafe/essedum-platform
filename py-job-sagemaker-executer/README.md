# Python Job Executor for AWS SageMaker

This directory contains the source code for the Python Job Executor, which is a component of the Essedum platform responsible for executing jobs on AWS SageMaker.

## Overview

The Python Job Executor for AWS SageMaker is a separate service that listens for job requests from the backend and executes them in a controlled environment on AWS SageMaker. This service is essential for running data processing, machine learning, and other Python-based tasks within the Essedum ecosystem on the AWS cloud.
...
- **AWS CLI**: The AWS Command-Line Interface for interacting with AWS resources.
...
2. **Create a Virtual Environment**:
   - Navigate to the `py-job-sagemaker-executer` directory.
...
2. **Install the Service**:
   - Open a command prompt with administrator privileges and run:
     ```bash
     nssm install "Python Job Executor for SageMaker"
     ```
   - In the NSSM GUI, configure the following:
     - **Path**: The full path to your `python.exe` (e.g., `C:\path\to\your\venv\Scripts\python.exe`).
     - **Startup directory**: The path to the `py-job-sagemaker-executer` directory.
     - **Arguments**: `app.py`.
     - **I/O Tab**: Redirect stdout and stderr to a log file for debugging.

3. **Start the Service**:
   - You can now start, stop, and manage the "Python Job Executor for SageMaker" service from the Windows Services panel (`services.msc`).
